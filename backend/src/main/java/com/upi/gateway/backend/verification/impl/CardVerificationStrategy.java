...
/**
 * Card verification strategy implementation
 * Simulates verification with card payment networks
 */
@Component
@Slf4j
public class CardVerificationStrategy implements VerificationStrategy {
    ...
    private static final long TIMEOUT_MS = 8000; // 8 seconds (cards take longer)
    private static final double SUCCESS_RATE = 0.88; // 88% success rate for card verification
    ...
    public VerificationResult verify(Transaction transaction) {
        ...
        try {
            // Simulate verification delay (cards take longer)
